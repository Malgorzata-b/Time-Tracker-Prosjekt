...
const updateTimeTracker = (activ) => {
  ...
  activity.forEach((t, i) => {
    ...
    t.querySelector(".Time").textContent = `${activity[activ].current}hrs`;
    t.querySelector(".Time-date").textContent =
      activ === "daily"
        ? `Yesterday - ${activity[activ].previous}hrs`
        : activ === "weekly"
...
const TimeTrackerContainer = () => {
  ...
  //Create Robson Div with img and title
  const robson = document.createElement("div");
  robson.classList.add("Robson");
  // Image Jeremy
  const image = document.createElement("img");
  image.src = "images/jeremy.png";
  // Create Raport
  const forJeremy = document.createElement("p");
  forJeremy.classList.add("for");
  forJeremy.textContent = "Report for";
  // Create name
  const nameJ = document.createElement("h1");
  nameJ.classList.add("Name");
  nameJ.textContent = "Jeremy Robson";
  // Appedning ROBSON DIV
  robson.append(image, forJeremy, nameJ);
  ...
  // Appending
  jeremy.append(robson, buttons);
  TimeTracker.append(jeremy);

  // Create activity cards
  activities.forEach((activities) => {
    //Create Div
    const iconDiv = document.createElement("div");
    iconDiv.classList.add("container-icon");
    iconDiv.classList.add(`${activities.title}`);
    iconDiv.innerHTML = activities.icon;
    ...
    titleElement.textContent = activities.title;
    //Create dots
    const dots = document.createElement("div");
    dots.classList.add("ellips");
    dots.innerHTML = ellipsiss;
    ...
    //Appending
    infoDiv.append(titleElement, dots, time, timeDate);
    iconDiv.append(infoDiv);
